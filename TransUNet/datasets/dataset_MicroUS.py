...
class MicroUS_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            if label is None:
                raise ValueError(f"Error loading label at {label_path}")
            if non_expert is None:
                raise ValueError(f"Error loading non-expert annotation at {non_expert_path}")

                # Now safely access the first channel and normalize
            image = image[:, :, 0] / 255.0
            label = label[:, :, 0] / 255.0
...
class cch(Dataset):
    def __init__(self, base_dir, list_dir, split, transform=None):
        ...
        self.label_list = open(os.path.join(list_dir, 'mask' + '.txt')).readlines()
        self.non_expert_list = open(os.path.join(list_dir, 'non_expert' + '.txt')).readlines()
        self.test_image_list = open(os.path.join(list_dir, 'cch_image' + '.txt')).readlines()
        self.test_label_list = open(os.path.join(list_dir, 'cch_mask' + '.txt')).readlines()
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            vol_image_name = self.test_image_list[idx].strip('\n')
            filepath = self.data_dir + '/cch_image' + "/{}.png".format(vol_image_name)
            vol_image = sitk.ReadImage(filepath)
            spacing = vol_image.GetSpacing()
            origin = vol_image.GetOrigin()
            direction = vol_image.GetDirection()
            ...
            vol_label_name = self.test_label_list[idx].strip('\n')
            filepath = self.data_dir + '/nci_isbi_mask' + "/{}.png".format(vol_label_name)
            vol_label = sitk.ReadImage(filepath)
            vol_label = sitk.GetArrayFromImage(vol_label)
